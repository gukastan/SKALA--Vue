...
const flowSteps = [
  { id: 1, title: '도시 카드 클릭', concept: '@click', description: '사용자가 여행할 도시에서 전주 카드를 클릭합니다.', code: '<WeatherCard @click="selectCity(city)" />' },
  ...
  { id: 3, title: 'selectedCity 변경', concept: 'ref', description: '반응형 상태가 부산에서 전주 객체로 변경됩니다.', before: '부산', after: '전주', code: 'const selectedCity = ref(weatherList.value[0])' },
  { id: 4, title: '날씨 조건 판별', concept: 'v-if · :class', description: '기온과 날씨 상태를 기준으로 화면에 필요한 조건을 판별합니다.', code: 'temp >= 25 ? "더움" : "선선함"' },
  { id: 5, title: '파생 상태 계산', concept: 'computed', description: '선택 도시의 날씨를 기준으로 펫 상태와 추천 행동을 다시 계산합니다.', code: 'const petState = computed(() => getPetState(selectedCity.value))' },
  { id: 6, title: '화면 데이터 전달', concept: 'props · emits', description: '도시와 펫 상태가 Pixel Pet 컴포넌트로 전달되고 돌봄 이벤트는 부모로 올라옵니다.', code: '<PetStage :city="selectedCity" :pet="pet" @care="handleCare" />' },
  { id: 7, title: '화면 자동 갱신', concept: 'Vue Reactivity', description: '반응성이 변경을 감지해 배경, PET STATUS, 메시지를 자동으로 갱신합니다.', before: '이전 화면', after: '전주 날씨 화면', code: 'selectedCity 변경 → template 자동 갱신' },
]
...
const technologyMap = [
  ...
  { name: 'computed', label: '파생 상태 계산', usage: '검색 결과 · 펫 상태 · 추천 행동', practice: 'ComputedBasic' },
  { name: ':class', label: '동적 스타일 연결', usage: '도시별 배경 · 날씨 상태', practice: 'VueBindClass' },
...
<template>
  <section class="learning-page">
    <header class="learning-intro">
      <p class="learning-intro-copy">구현원리를 단계별로 살펴봅시다.</p>
    </header>

    <div class="learning-main">
      <el-card class="flow-map-card" shadow="never">
        <div class="learning-section-heading"><div><el-tag type="primary" effect="light">실행 흐름</el-tag><h2>전주 여행 시작</h2></div><span>단계를 눌러보세요</span></div>
